...
def view(date):
	...
	if request.method=='POST':
		...
		curs=conaf.cursor()
		arg=(fooditem,date)
		curs.execute("INSERT into food_date (food_id,log_date_id) values(%s,%s)",arg)
		conaf.commit()
		conaf.close()
	host="localhost";user="root";dbname="diettracker"
	conn = pymysql.connect(host, user=user,port=3306,passwd="", db=dbname)
	...
	cur12=conn12.cursor()
	cur12.execute("Select food.name,food.protein,food.carbohydrates,food.fat,food.calories from food,food_date,log_date where food.id=food_date.food_id and log_date.entry_date=food_date.log_date_id and log_date.entry_date='2020-05-09'; ")
	...
	return render_template('day.html',date=pretty_results,foods=food_list,food_dets=list_of_food_details_for_the_day,total_list=list_of_total)
